...
contract AgencyRegistry is Ownable, ReentrancyGuard {
    ...
    event AgencyRegistered(
        uint256 indexed agencyId, address indexed owner, string companyName, bytes32 gstNumberHash, uint256 stakeAmount
    );
     ...
     */
    function registerAgency(string memory companyName, bytes32 gstHash) external nonReentrant returns (uint256) {
        ...
        require(stablecoin.transferFrom(msg.sender, address(this), STAKE_AMOUNT), "Stake transfer failed");
    ...
    function getAgency(uint256 agencyId)
        external
        view
        returns (
            address owner,
