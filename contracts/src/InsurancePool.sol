...
contract InsurancePool is Ownable, ReentrancyGuard {
    ...
    event InsurancePurchased(uint256 indexed policyId, address indexed policyholder, uint256 indexed projectId, uint256 coverageAmount, uint256 premium);
    ...
    function getPolicy(uint256 policyId) external view returns (
        address policyholder, uint256 projectId, uint256 coverageAmount, uint256 premiumPaid, PolicyStatus status, uint256 expiryTime
    ) {
        InsurancePolicy storage policy = policies[policyId];
        return (policy.policyholder, policy.projectId, policy.coverageAmount, policy.premiumPaid, policy.status, policy.expiryTime);
    }
    ...
    function getPoolMetrics() external view returns (uint256 premiumsCollected, uint256 claimsPaid, uint256 currentBalance, uint256 profit) {
        uint256 balance = stablecoin.balanceOf(address(this));
        return (totalPremiumsCollected, totalClaimsPaid, balance, totalPremiumsCollected > totalClaimsPaid ? totalPremiumsCollected - totalClaimsPaid : 0);
    }
