...
contract AIOracle is Ownable {
    ...
    event JobRequested(
        uint256 indexed jobId, JobType indexed jobType, uint256 referenceId, bytes32 requestHash, bytes data
    );
    ...
    function fulfillSkillGrade(uint256 jobId, uint256 submissionId, address user, uint8 score, string memory report)
        external
        onlyOwner
    {
        VerificationJob storage job = jobs[jobId];
    ...
    {
        VerificationJob storage job = jobs[jobId];
        return (job.jobType, job.status, job.requestHash, job.requester, job.referenceId, job.timestamp);
    }
}
